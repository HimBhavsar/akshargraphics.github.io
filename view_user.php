<?php
    ...
    function search()
    {
        $result = mysql_query("SELECT u_name FROM user_table");
        while ($row = mysql_fetch_assoc($result)) {
            echo "<div id='link' onClick='addText(\"".$row['u_name']."\");'>" . $row['u_name'] . "</div>";  
        }
    }
?>
<!DOCTYPE html>
                ...
                <nav class="navbar navbar-expand-lg navbar-absolute fixed-top navbar-transparent">
                    <div class="container-fluid">
                        <div class="navbar-wrapper">
                            <a class="navbar-brand" href="view_user.php">Users Details</a>
                        </div>
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <?php
                                            ...
                                            if(isset($_GET['deleteid'])){
                                                ...
                                                $deletequery = $mysqlicon->query("delete from user_table where u_id='{$deleteid}'") or die("Error in Query".$mysqlicon->error);
                                            }
                                        ?>
                                        <table class="table">
                                            ...
                                            <?php
                                                $query = $mysqlicon->query("select * from  user_table") or die("Error in Query".$mysqlicon->error); 
                                                while ($row = $query->fetch_array()) {
                                                    echo "<tr>";
                                                    echo "<td>{$row['u_id']}</td><td>{$row['u_name']}</td><td>{$row['u_email']}</td>";
                                                    echo "<td><a Onclick='return ConfirmDelete();' href='?deleteid={$row['u_id']}'>Delete</a></td>";
                                                    echo "</tr>";
                                                }                   
                                            ?>
                                        </table>
