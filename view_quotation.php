                ...
                <nav class="navbar navbar-expand-lg navbar-absolute fixed-top navbar-transparent">
                    <div class="container-fluid">
                        <div class="navbar-wrapper">
                            <a class="navbar-brand" href="view_quotation.php">Quotation Details</a>
                        </div>
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <?php
                                            ...
                                            if(isset($_GET['deleteid'])){
                                                ...
                                                $deletequery = $mysqlicon->query("delete from quotation_table where q_id='{$deleteid}'") or die("Error in Query".$mysqlicon->error);
                                            }
                                        ?>
                                        <table class="table">
                                            ...
                                            <?php
                                                $query = $mysqlicon->query("SELECT
                                                    quotation_table.q_id
                                                    , client_table.c_name
                                                    , material_table.mtr_type
                                                    , material_table.mtr_pricePerFeet
                                                    , material_table.mtr_designCharge
                                                    , quotation_table.q_labourCharge
                                                    , quotation_table.q_shiftingCharge
                                                    , quotation_table.q_subTotal
                                                    , quotation_table.q_GST
                                                    , quotation_table.q_grandTotal
                                                FROM
                                                    akshar_graphics_db.quotation_table
                                                    INNER JOIN akshar_graphics_db.client_table 
                                                        ON (quotation_table.c_id = client_table.c_id)
                                                    INNER JOIN akshar_graphics_db.material_table 
                                                        ON (material_table.mtr_id = quotation_table.mtr_id) ORDER BY quotation_table.q_id;")
                                                ...
                                                while ($row = $query->fetch_array()) {
                                                    ...
                                                    . "<td>{$row['c_name']}</td>"
                                                    . "<td>{$row['mtr_type']}</td>"
                                                    . "<td>{$row['mtr_pricePerFeet']}</td>"
                                                    . "<td>{$row['mtr_designCharge']}</td>"
                                                    . "<td>{$row['q_labourCharge']}</td>"
                                                    . "<td>{$row['q_shiftingCharge']}</td>"
                                                    . "<td>{$row['q_subTotal']}</td>"
                                                    . "<td>{$row['q_GST']}</td>"
                                                    . "<td>{$row['q_grandTotal']}</td>";
                                                    echo "<td><a href='edit_quotation.php?eid={$row['q_id']}'>Edit </a>| <a Onclick='return ConfirmDelete();' href='?deleteid={$row['q_id']}'>Delete</a></td>";
                                                    echo "</tr>";
                                                }                   
                                            ?>
                                        </table>
