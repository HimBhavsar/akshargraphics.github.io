...
$mysqlicon = connection_open();
    ...
    if ($_POST) {
        ...
        $query = $mysqlicon->query("UPDATE `meeting_table` SET `mtg_date`='{$mtg_date}',`mtg_place`='{$mtg_place}',`mtg_type`='{$mtg_type}',`v_id`='{$v_id}' WHERE mtg_id='{$a}'")
                or die("Error in Query Using Die" . $mysqlicon->error);
        if ($query) {
            // header ('Location:disp_attendance_table.php');
            echo "<script>alert('Record updated');window.location='view_meeting.php';</script>";
        }
    }
    $fetch_query = $mysqlicon->query("select * from meeting_table where mtg_id='{$a}'") or die("Error in Query" . $mysqlicon->error);
      ...
      <nav class="navbar navbar-expand-lg navbar-absolute fixed-top navbar-transparent">
          <div class="container-fluid">
                        <div class="navbar-wrapper">
                            <a class="navbar-brand" href="edit_meeting.php">Update Meeting Details</a>
                        </div>
                  ...
                  <div class="row">
                    <div class="col-md-11 pr-1 px-1">
                        ...
                        <?php
                            ...
                            $v_name = $mysqlicon->query("select * from  visitor_table where v_id='{$fetchdata['v_id']}' ") or die("Error in Query" . $mysqlicon->error);
                            $fetchuser = $v_name->fetch_array();
                            echo "<option value='{$fetchuser['v_id']}'>{$fetchuser['v_name']}</option>";
                  ...
                  <div class="row">
                    <div class="col-md-4 pr-1">
                      <div class="form-group">
                        ...
                        <input id="form_name" name="mtg_place" type="text" class="form-control" placeholder="Place of Meeting" value="<?php echo $fetchdata['mtg_place']; ?>">
                      </div>
                    </div>
                    ...
                    <div class="col-md-5 px-1">
                        <div class="form-group">
                            ...
                            <input id="form_name" name="mtg_date" type="date" class="form-control" value="<?php echo $fetchdata['mtg_date']; ?>">
                        </div>
                    </div>  
                  </div>
                  <div class="row">
