                ...
                <nav class="navbar navbar-expand-lg navbar-absolute fixed-top navbar-transparent">
                    <div class="container-fluid">
                        <div class="navbar-wrapper">
                            <a class="navbar-brand" href="view_order.php">Order Details</a>
                        </div>
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <?php
                                            ...
                                            if(isset($_GET['deleteid'])){
                                                ...
                                                $deletequery = $mysqlicon->query("delete from order_table where o_id='{$deleteid}'") or die("Error in Query".$mysqlicon->error);
                                            }
                                        ?>
                                        <table class="table">
                                            ...
                                            <?php
                                                $query = $mysqlicon->query("SELECT
                                                                ...
                                                                , order_table.o_paymentType
                                                                , order_table.o_paymentDate
                                                                , order_table.o_status
                                                            FROM
                                                                akshar_graphics_db.client_table
                                                                INNER JOIN akshar_graphics_db.order_table 
                                                                    ON (client_table.c_id = order_table.c_id)
                                                                INNER JOIN akshar_graphics_db.material_table 
                                                                    ON (material_table.mtr_id = order_table.mtr_id)
                                                                INNER JOIN akshar_graphics_db.quotation_table 
                                                                    ON (quotation_table.q_id = order_table.q_id);")
                                                ...
                                                while ($row = $query->fetch_array()) {
                                                    ...
                                                    . "<td>{$row['o_paymentType']}</td>"
                                                    . "<td>{$row['o_paymentDate']}</td>"
                                                    . "<td>{$row['o_status']}</td>";
                                                    echo "<td><a href='edit_order.php?eid={$row['o_id']}'>Edit </a>| <a Onclick='return ConfirmDelete();' href='?deleteid={$row['o_id']}'>Delete</a></td>";
                                                    echo "</tr>";
                                                }                   
                                            ?>
                                        </table>
